...
class AController: WKInterfaceController {
    ...
    override func awake(withContext context: Any?) {
        ...
        countLabel.setText(String(defaults.integer(forKey: "A")))
    }
    ...
    @IBAction func plusPressed() {
        let number = defaults.integer(forKey: "A")
        defaults.set(number+1, forKey: "A")
        print(defaults.integer(forKey: "A"))
    ...
    @IBAction func renamePressed() {
        presentTextInputController(withSuggestions: [""], allowedInputMode:   WKTextInputMode.plain) { (arr: [Any]?) in
            self.defaults.set(arr?.first, forKey:"AName")
        }
    }
