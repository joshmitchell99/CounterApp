...
class DController: WKInterfaceController {
    ...
    override func awake(withContext context: Any?) {
        ...
        countLabel.setText(String(defaults.integer(forKey: "D")))
    }
    ...
    @IBAction func plusPressed() {
        let number = defaults.integer(forKey: "D")
        defaults.set(number+1, forKey: "D")
        print(defaults.integer(forKey: "D"))
    ...
    @IBAction func renamePressed() {
        presentTextInputController(withSuggestions: [""], allowedInputMode:   WKTextInputMode.plain) { (arr: [Any]?) in
            self.defaults.set(arr?.first, forKey:"DName")
        }
    }
