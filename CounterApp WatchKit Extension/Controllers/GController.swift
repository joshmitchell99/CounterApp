...
class GController: WKInterfaceController {
    ...
    override func awake(withContext context: Any?) {
        ...
        countLabel.setText(String(defaults.integer(forKey: "G")))
    }
    ...
    @IBAction func plusPressed() {
        let number = defaults.integer(forKey: "G")
        defaults.set(number+1, forKey: "G")
        print(defaults.integer(forKey: "G"))
    ...
    @IBAction func renamePressed() {
        presentTextInputController(withSuggestions: [""], allowedInputMode:   WKTextInputMode.plain) { (arr: [Any]?) in
            self.defaults.set(arr?.first, forKey:"GName")
        }
    }
