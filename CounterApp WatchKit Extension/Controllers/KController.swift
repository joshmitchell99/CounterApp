...
class KController: WKInterfaceController {
    ...
    override func awake(withContext context: Any?) {
        ...
        countLabel.setText(String(defaults.integer(forKey: "K")))
    }
    ...
    @IBAction func plusPressed() {
        let number = defaults.integer(forKey: "K")
        defaults.set(number+1, forKey: "K")
        print(defaults.integer(forKey: "K"))
    ...
    @IBAction func renamePressed() {
        presentTextInputController(withSuggestions: [""], allowedInputMode:   WKTextInputMode.plain) { (arr: [Any]?) in
            self.defaults.set(arr?.first, forKey:"KName")
        }
    }
