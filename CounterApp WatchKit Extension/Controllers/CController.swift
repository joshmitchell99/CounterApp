...
class CController: WKInterfaceController {
    ...
    override func awake(withContext context: Any?) {
        ...
        countLabel.setText(String(defaults.integer(forKey: "C")))
    }
    ...
    @IBAction func plusPressed() {
        let number = defaults.integer(forKey: "C")
        defaults.set(number+1, forKey: "C")
        print(defaults.integer(forKey: "C"))
    ...
    @IBAction func renamePressed() {
        presentTextInputController(withSuggestions: [""], allowedInputMode:   WKTextInputMode.plain) { (arr: [Any]?) in
            self.defaults.set(arr?.first, forKey:"CName")
        }
    }
}
