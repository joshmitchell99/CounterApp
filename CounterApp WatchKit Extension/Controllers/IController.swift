...
class IController: WKInterfaceController {
    ...
    override func awake(withContext context: Any?) {
        ...
        countLabel.setText(String(defaults.integer(forKey: "I")))
    }
    ...
    @IBAction func plusPressed() {
        let number = defaults.integer(forKey: "I")
        defaults.set(number+1, forKey: "I")
        print(defaults.integer(forKey: "I"))
    ...
    @IBAction func renamePressed() {
        presentTextInputController(withSuggestions: [""], allowedInputMode:   WKTextInputMode.plain) { (arr: [Any]?) in
            self.defaults.set(arr?.first, forKey:"IName")
        }
    }
