...
class FController: WKInterfaceController {
    ...
    override func awake(withContext context: Any?) {
        ...
        //countLabel.setText(String(defaults.integer(forKey: "F")))
    }
    ...
    @IBAction func plusPressed() {
        let number = defaults.integer(forKey: "F")
        defaults.set(number+1, forKey: "F")
        print(defaults.integer(forKey: "F"))
    ...
    @IBAction func renamePressed() {
        presentTextInputController(withSuggestions: [""], allowedInputMode:   WKTextInputMode.plain) { (arr: [Any]?) in
            self.defaults.set(arr?.first, forKey:"FName")
        }
    }
